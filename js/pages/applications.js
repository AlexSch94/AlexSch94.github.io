...
;(function () {
    ...
    function switchObservers(size) {
        if (size === 'large') {
            // Sections
            sections.forEach((section) => largeSectionObserver.observe(section))
            sections.forEach((section) => mediumSectionObserver.unobserve(section))
            sections.forEach((section) => smallSectionObserver.unobserve(section))

            // Advantages
            advantageWrappers.forEach((advantageWrapper) => largeAdvantageObserver.observe(advantageWrapper))
            advantageWrappers.forEach((advantageWrapper) => mediumAdvantageObserver.unobserve(advantageWrapper))
        } else if (size === 'medium') {
            // Sections
            sections.forEach((section) => largeSectionObserver.unobserve(section))
            sections.forEach((section) => mediumSectionObserver.observe(section))
            sections.forEach((section) => smallSectionObserver.unobserve(section))

            // Advantages
            advantageWrappers.forEach((advantageWrapper) => largeAdvantageObserver.unobserve(advantageWrapper))
            advantageWrappers.forEach((advantageWrapper) => mediumAdvantageObserver.observe(advantageWrapper))
        } else {
            sections.forEach((section) => largeSectionObserver.unobserve(section))
            sections.forEach((section) => mediumSectionObserver.unobserve(section))
            sections.forEach((section) => smallSectionObserver.observe(section))
